...
let newNumbersArr = numbers.map((newNum) => {
  return newNum / 2
})
...
let iCities = cities.filter((cityNames) => {
  return cityNames.endsWith('i')
})
...
/* 
    Using forEach method, identify each element of the 'fruits' array above if it's an apple or orange by its color. If the color is red, print "The fruit with index [THE ELEMENT'S INDEX] is an 
    apple." If the color is orange, print "The fruit with index [THE ELEMENT INDEX] is an orange." If the color is not red or apple, print "The fruit with index [THE ELEMENT'S INDEX] is 
    neither apple or orange."
...
fruits.forEach((fruit, ind) =>{
  ...
  console.log(`The fruit with index ${ind} is an apple`)
    ...
    console.log(`The fruit with index ${ind} is neither an apple or an orange`)
  }
}) 
...
// The fruit with index 0 is neither an apple or an orange
// The fruit with index 1 is an orange
// The fruit with index 2 is neither an apple or an orange
// The fruit with index 3 is an apple
...
let totalRicePrice = foods.filter((rice) => rice.tags.includes('rice')).reduce((acc, cur) => acc + cur.price, 0)
