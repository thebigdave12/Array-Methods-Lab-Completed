...
const postTaxPrices = prices.map((pricesAfterTax) =>{
  return pricesAfterTax = pricesAfterTax + (pricesAfterTax * .07)
})
...
const myStrongest = monstersInYourPocket.filter((monsterArr) => {
  return monsterArr.CP > 200
})
...
const orderTotalsAfterTax = orders.map((totals) => {
  return totals.price + (totals.price * totals.tax)
})
...
const bobsTotalPrice = purchases.filter((purchaser) => purchaser.owner === 'Bob').reduce((acc, cur) => acc + cur.price, 0)
